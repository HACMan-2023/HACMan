...
def evaluate_policy(
    ...
    verbose_buffer = []
    if per_object_succ:
        per_object_results = defaultdict(list)
    ...
    if return_episode_rewards:
        return episode_rewards, episode_lengths, success_buffer
    
    if len(success_buffer) > 0:
        success_rate = np.average(success_buffer)

    if per_object_succ:
        return mean_reward, std_reward, success_rate, per_object_results
    else:
        if verbose:
            return mean_reward, std_reward, success_rate, verbose_buffer
        else:
